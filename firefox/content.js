...
class ShadowGagChecker {
  ...
  async init() {
    ...
    try {
      ...
      try {
        const stateResponse = await chrome.runtime.sendMessage({ action: 'getExtensionState' });
        if (stateResponse) {
          this.isEnabled = stateResponse.enabled;
          console.log('ShadowGag: Initial state from background:', this.isEnabled);
        }
      } catch (error) {
        console.log('ShadowGag: Could not get initial state from background:', error);
      }
  ...
  // Load settings from storage
  async loadSettings() {
    console.log('ShadowGag: Loading settings from storage...');
  ...
  executeCustomFunction(element, customCode, type) {
    ...
    try {
      ...
      // Function constructor is used for user-defined custom styling functions
      // This allows users to create personalized visual markers beyond predefined options
      // Code is user-written only, stored locally, never transmitted externally
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'setEnabled') {
    console.log('ShadowGag: Set enabled state to:', request.enabled);
    if (window.shadowGagChecker) {
  ...
  if (request.action === 'settingsChanged') {
    console.log('ShadowGag: Settings changed:', request.settings);
    if (window.shadowGagChecker) {
      window.shadowGagChecker.updateSettings(request.settings);
